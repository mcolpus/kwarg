            ...
            if (node->type == ARGRECOMBINATION)
            {
                if (node->predecessor.two.prefix.target == -1)
                    ...
                    for (j = 0; j < n; j++)
                        if (edges[j] >> 1 == i)
                        {
                            node->predecessor.two.prefix.target = arg_addnode(arg, ARGANCESTOR, NULL, sequence[j]);
                            sequence[j] = NULL;
                            break;
                        }
                if (node->predecessor.two.suffix.target == -1)
                    ...
                    for (j = 0; j < n; j++)
                        if (edges[j] >> 1 == i)
                        {
                            node->predecessor.two.suffix.target = arg_addnode(arg, ARGANCESTOR, NULL, sequence[j]);
                            sequence[j] = NULL;
                            break;
                        }
            }
...
{
    PartialHistory new_history = {
        .g = history->g,
        .event_list = history->event_list,
        .sequence_labels = history->sequence_labels,
        .site_labels = history->site_labels,
        .num_of_sequences = history->num_of_sequences,
        .recombinations = history->recombinations,
        .recurrent_mutations = history->recurrent_mutations,
...
/**
 * Used to deep copy source to target, except the event_list is made empty
 */
void copy_partial_history_without_eventlist(PartialHistory *source, PartialHistory *target)
{
    target->g = copy_genes(source->g);
    if (source->event_list != NULL)
        target->event_list = MakeLList();
    if (source->sequence_labels != NULL)
    {
        target->sequence_labels = elist_make();
        elist_safeextend(target->sequence_labels, source->sequence_labels);
    }
    if (source->site_labels != NULL)
    {
        target->site_labels = elist_make();
        elist_safeextend(target->site_labels, source->site_labels);
    }
}
...
PartialHistory *history_fragment_to_partial_history(HistoryFragment *fragment)
{
    PartialHistory history = {
        .g = fragment->g,
        .event_list = fragment->event_list,
        .sequence_labels = fragment->sequence_labels,
        .site_labels = fragment->site_labels
    };
    return &history;
}

void free_history_fragment(HistoryFragment *fragment)
{
    free_genes(fragment->g);
    if (fragment->event_list != NULL)
    {
        while (Length(fragment->event_list) != 0)
            free(Pop(fragment->event_list));
        DestroyLList(fragment->event_list);
    }
    if (fragment->sequence_labels != NULL)
    {
        elist_destroy(fragment->sequence_labels);
    }
    if (fragment->site_labels != NULL)
    {
        elist_destroy(fragment->site_labels);
    }
    free(fragment);
}
