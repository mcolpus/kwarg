...
int g_exact_randomise = 0;    /* Random evolutionary histories? */
static int _reusable = 0;    /* Is hash table reusable? */
static int _skip_lookup = 0; /* Should LOOKUP event be allowed? */
                                     ...
                                     HashTable *t, int target)
{
    ...
    for (i = 0; i < elist_length(genes); i++)
    {
        ...
        if (hashtable_lookup((void *)p, t, &lookup))
        {
            ...
            if (_reusable && (prevtarget < 0) && (base - target <= prevtarget + 1))
            {
                ...
                if (!_skip_lookup)
                {
                    ...
                    if (g_eventlist != NULL)
                    {
                        Append(g_eventlist, splits[*n].g->event_list);
                        e = (Event *)xmalloc(sizeof(Event));
                        e->type = LOOKUP;
                        e->event.lookup = -prevtarget - 1;
                        Enqueue(g_eventlist, (void *)e);
                    }
                    free(splits[*n].g);
                    ...
                    for (i++; i < elist_length(genes); i++)
                    {
                        ...
                        if (g_eventlist != NULL)
                        {
                            while (Length(splits[*n].g->event_list) > 0)
                                free(Pop(splits[*n].g->event_list));
                            DestroyLList(splits[*n].g->event_list);
                        }
                    ...
                    for (i = 0; i < *n; i++)
                    {
                        ...
                        if (g_eventlist != NULL)
                        {
                            while (Length(splits[i].g->event_list) > 0)
                                free(Pop(splits[i].g->event_list));
                            DestroyLList(splits[i].g->event_list);
                        }
                        free(splits[i].g);
                    }
                    elist_destroy(genes);
            ...
            else if ((_reusable && ((2 * (target - base) + (base > 0) < prevtarget) || (base - target > prevtarget + 1))) || (!_reusable && (target - base <= prevtarget)))
            {
            ...
            else
            {
                ...
                hashtable_update((void *)p,
                                 (void *)((target - base) << _reusable), t, NULL);
                free_packedgenes(p);
            }
        }
        else
        ...
        splits[*n].am = count_ancestral_material(g);
...
static void free_elist_elements(EList *elist)
{
    ...
    for (i = 0; i < elist_length(elist); i++)
    {
        ...
        if ((g_eventlist != NULL) && (s->event_list != NULL))
        {
            while (Length(s->event_list) > 0)
                free(Pop(s->event_list));
            DestroyLList(s->event_list);
        }
        free_genes(s->g);
        free(s);
    }
...
static int check_for_bottom(EList *genes)
{
    ...
    for (i = 0; i < elist_length(genes); i++)
    {
        ...
        if (no_recombinations_required(g))
        {
            ...
            if (g_eventlist != NULL)
            {
                Append(g_eventlist, s->event_list);
                s->event_list = NULL;
            }
 ...
 */
static void _coalesce_cande_recursion(LList *stack, EList *component, PartialHistory *history,
                                      EList *E, int **C, int *components,
                                      void (*f)(PartialHistory *g))
{
    int i, j, n = 0, old;
    ...
    if (Length(stack) == 0)
    {
        ...
        int has_coalescence_been_encountered = 0;
        // Genes *old_genes = NULL;
        // LList *oldevents = history->event_list;
        // EList *oldelements = history->sequence_labels;
        // EList *oldsites = history->site_labels;

        /* Coalesce in reverse order to avoid having to keep track of
         * where other sequences are moved when sequences disappear by
         * coalescence.
         */
        for (i = history->g->n - 1; i >= 0; i--)
        {
            if (components[i] - 1 != i)
            {
                /* Coalesce as specified by components */
                if (!has_coalescence_been_encountered)
                {
                    /* First coalescence encountered; create structure for
                     * carrying out the coalescences in.
                     */
                    has_coalescence_been_encountered = 1;
                ...
                coalesce(new_history, components[i] - 1, i);
                if (new_history->event_list != NULL)
                {
                    ...
                    event->event.c.s2 = i;
                    Enqueue(new_history->event_list, event);
                }
            }
        }
        if (has_coalescence_been_encountered)
        {
             ...
             */
            implode_genes(new_history->g);
            f(new_history); // most likely storing to a history fragment

            //TODO: might need to free new_history. Likewise in seqerror_flips, recmut_flips, maximal_prefix_coalesces_map
            //maximal_postfix_coalesces_map, maximal_infix_coalesces_map, maximal_overlap_coalesces_map
        }
    }
    else
    {
        ...
        current = Pop(stack);
        if ((components[current->node] <= 0) && (components[current->node] != -current->class))
        {
            ...
            if (i == elist_length(component))
            {
                ...
                _coalesce_cande_recursion(stack, component, history, E, C, components, f);
                /* Clean up */
                if (elist_length(E + current->node) > 0)
                {
                    for (i = 0; i < n; i++)
 ...
 */
static void _coalesce_compatible_and_entangled_map(PartialHistory *history, void (*f)(PartialHistory *))
{
    Genes *g = history->g;
    int i, j;
                            ...
                            int try_coalesces)
{
    ...
    if (target > 0)
    {
        ...
        if (!g_exact_randomise && check_for_bottom(prefix))
        {
            free(start);
            free(end);
            if (try_coalesces)
        ...
        if (target > 1)
        {
            ...
            if (!g_exact_randomise && check_for_bottom(infix))
            {
                free(start);
                free(end);
                if (try_coalesces)
            ...
            if (n > 0)
            {
                ...
                if (!g_exact_randomise && transfer2splitinformation(splits, infix, 2, &i, t, target))
                {
                    free(start);
                    free(end);
                    if (try_coalesces)
        ...
        if (n > 0)
        {
            if (!g_exact_randomise && transfer2splitinformation(splits, prefix, 1, &i, t, target))
            {
                free(start);
                free(end);
                if (try_coalesces)
    ...
    if (i > 0)
    {
        ...
        for (j = 0; j < i; j++)
        {
                                 ...
                                 (splits[j].splits > 0)))
            {
                ...
                if (g_eventlist != NULL)
                    Prepend(splits[j].g->event_list, g_eventlist);
                ...
                for (; j < i; j++)
                {
                    ...
                    if (g_eventlist != NULL)
                    {
                        while (Length(splits[j].g->event_list) > 0)
                            free(Pop(splits[j].g->event_list));
                        DestroyLList(splits[j].g->event_list);
                    }
                    free(splits[j].g);
                }
                free(splits);
 ...
 * of recombinations is compiled in g_eventlist. If haploblocks is not
                       ...
                       HashTable *t)
{
    ...
    void *lookup;
    LList *implode, *tmp = g_eventlist;
    Event *e;
#ifdef HAPLOTYPE_BLOCKS
    int i, j, n, localbound, oldreusable = _reusable;
        ...
        if (hashtable_lookup((void *)p, t, &lookup))
        {
            ...
            if (bound < 0)
            {
                ...
                if (!_skip_lookup)
                {
                    ...
                    if ((g_eventlist != NULL) && (-bound <= upper))
                    {
                        e = (Event *)xmalloc(sizeof(Event));
                        e->type = LOOKUP;
    ...
    if (!no_recombinations_required(g))
    {
        ...
        if (print_progress != NULL)
            fprintf(print_progress, "At least %d recombination%s required\n", bound,
                    (bound != 1 ? "s" : ""));
        p = pack_genes(g);
        if (hashtable_update((void *)p, (void *)(bound << _reusable),
                             t, NULL) < 0)
        {
            r1 = 1;
        }
        for (; (bound <= upper) && !beagle_recursion(g, t, bound, 1);)
        {
            ...
            if (print_progress != NULL)
                fprintf(print_progress, "At least %d recombinations required\n",
                        bound);
#endif
            hashtable_update((void *)p, (void *)(bound << _reusable),
                             t, NULL);
        }
            ...
            for (i = 2; i < g->length; i++)
                for (j = 0; j <= g->length - i; j++)
                {
                    ...
                    if (!no_recombinations_required(h))
                    {
                        ...
                        r2 = 0;
                        lookup = (void *)0;
                        if (!hashtable_lookup((void *)p, t, &lookup) || ((int)lookup >= 0))
                        {
                            ...
                            if (i > 2)
                                localbound = _intmax(localbound,
                                                     _intmax(haploblocks[j][i - 3],
                                                             haploblocks[j + 1][i - 3]));
                            if (hashtable_update((void *)p, (void *)(localbound << _reusable),
                                                 t, NULL) < 0)
                                r2 = 1;
                            for (; (localbound <= upper) && !beagle_recursion(h, t, localbound, 1);)
                                hashtable_update((void *)p, (void *)(++localbound << _reusable),
                                                 t, NULL);
...
static void _reset_builtins(Genes *g)
{
    ...
    _am = count_ancestral_material(g);
    _seq = g->n;
    _len = g->length;
    ...
    if (_greedy_functioncalls != NULL)
    {
        ...
        _greedy_functioncalls = hashtable_new(6, (unsigned long (*)(void *, void *))_greedy_hash,
