                ...
                for (j = 0; j <= g->length - i; j++)
                {
                    ...
                    if (!no_recombinations_required(h))
                    {
                        ...
                        if (!hashtable_lookup((void *)p, t, &lookup) || ((intptr_t)lookup >= 0))
                        {
                            ...
                            haploblocks[j][i - 2] = -(intptr_t)lookup - 1;
                        if (!r2)
                            free_packedgenes(p);
                    }
                    free_genes(h);
    ...
    while (h->n > 1)
    {
        ...
        for (i = 0; i < elist_length(configurations); i++)
        {
            ...
            if (beagle_reusable_bounded(h, NULL, 0, r - (e->type == RECOMBINATION), t) <= r - (e->type == RECOMBINATION))
            {
                ...
                {
                    free_genes((Genes *)elist_get(configurations, permutation[i]));
...
double ggreedy(Genes *g, FILE *print_progress, int (*select)(double), void (*reset)(void), int ontheflyselection)
{
    ...
    if (lookup != NULL)
    {
        if ((intptr_t)elist_get(lookup, 0) == INT_MAX)
    ...
    while (!_choice_fixed)
    {
        ...
        auto action = [&](Genes *g)
        {
            ...
            {
                fprintf(stderr, "Error: number of sequence labels in g_sequence_labels [%d] not equal to current size of dataset [%d]. Event type: %.1f", elist_length(g_sequence_labels), g->n, _recombinations);
                exit(0);
            }
        ...
        case COAL:
            break;
        case SE:
            seflips = seflips + greedy_choice->recombinations / se_cost;
            break;
        case RM:
        ...
        {
            if (seflips + rmflips > (intptr_t)elist_get(lookup, recombs))
            {
                bad_soln = 1;
                break;
            }
