...
static void marginal_trees(ARG *arg, AnnotatedGenes *a, FILE *fp,
                           ...
                           char *label)
{
  int i, j, next, *interval_starts, *tree_nodes, (*descendant)[2];
  ARG *tree;
  char *t;
  MyString *s;
  ...
  if (intervals)
  {
    ...
    interval_starts = (int *)xcalloc(a->g->length, sizeof(int));
    interval_starts[0] = 1;
    for (i = 0; i < arg->n; i++)
      if (arg->nodes[i].type == ARGRECOMBINATION)
        interval_starts[arg->nodes[i].predecessor.two.position] = 1;

    /* Convert Booleans to index of next interval start */
    next = a->g->length;
    for (i = a->g->length - 1; i >= 0; i--)
      if (interval_starts[i])
      {
        interval_starts[i] = next;
        next = i;
      }
  }
  else
  {
    ...
    interval_starts = (int *)xmalloc(a->g->length * sizeof(int));
    for (i = 0; i < a->g->length; i++)
      interval_starts[i] = i + 1;
  }
  ...
  tree->n = 0;
  tree_nodes = (int *)xmalloc(arg->n * sizeof(int));
  ...
  while (i < a->g->length)
  {
    ...
    for (j = 0; j < arg->n; j++)
      if (arg->nodes[j].type == ARGSAMPLE)
        visit(arg, tree, j, descendant, tree_nodes, i, interval_starts[i]);

    /* Copy sequence to tree nodes */
    for (j = 0; j < arg->n; j++)
      if ((tree_nodes[j] != -1) && (arg->nodes[j].sequence != NULL))
      {
        /* Copy relevant part of the sequence from ARG node to tree node */
        tree->nodes[tree_nodes[j]].sequence = (char *)xmalloc((interval_starts[i] - i + 1) * sizeof(char));
        strncpy(tree->nodes[tree_nodes[j]].sequence, arg->nodes[j].sequence,
                interval_starts[i] - i);
        tree->nodes[tree_nodes[j]].sequence[interval_starts[i] - i] = '\0';
      }
    ...
    if (a->positions != NULL)
    {
      ...
      if (i < interval_starts[i] - 1)
      {
        /* Tree valid for interval rather than single site */
        mystring_addend(s, '-');
        mystring_append(s, GetByIndex(a->positions, interval_starts[i] - 1));
      }
    }
    else
    {
                ...
                ARGLabels nodelabels, int annotate_edges, int generate_id, ...)
{
  ...
  case ARGGML:
    ...
    break;
  case YAML:
    fprintf(fp, "this is a tskit header\n");
    break;
  }
  ...
  for (i = 0; i < arg->n; i++)
  {
    ...
    case ARGGML:
      ...
      break;
    case YAML:
      fprintf(fp, "node - id: %d label: \"", i);
      break;
    }
    /* Generate node label */
    if (((arg->nodes[i].label != NULL) || ((arg->nodes[i].type == ARGSAMPLE) && generate_id)) &&
        ((nodelabels == ARGLABEL) || (nodelabels == ARGBOTH) || (nodelabels == ARGLABELFIRST) || ((nodelabels == ARGSEQUENCEFIRST) && (arg->nodes[i].sequence == NULL))))
    {
      if (arg->nodes[i].label != NULL)
...
{
  fprintf(fp, "header: arg exported to text\n");
  fprintf(fp, "number_of_nodes: %d\n", arg->n);

  if (arg->n > 0)
  {
    fprintf(fp, "sequence_length: %d\n", strlen(arg->nodes[0].sequence));
  }
  else
  {
    fprintf(fp, "sequence_length: 0\n");
  }
  ...
  for (i = 0; i < arg->n; i++)
  {
    ...
    case ARGCOALESCENCE:
      // One outgoing edge
      fprintf(fp, "  - source: %d\n", arg->nodes[i].predecessor.one.target);
      fprintf(fp, "    target: %d\n", i);
      fprintf(fp, "    type: single\n");
      fprintf(fp, "    position: 0\n");

      muts = arg->nodes[i].predecessor.one.mutations;
      if (muts != NULL && Length(muts) > 0)
      {
        LListCounter *lcounter = MakeCounter(muts, FIRST);
        int j = 0;

        fprintf(fp, "    mutations:\n");
        for (j = 0; j < Length(muts); j++)
        {
          p = (int)Next(lcounter);
          if (p == -INT_MAX)
            fprintf(fp, "      - %d\n", 0);
    ...
    case ARGRECOMBINATION:
      ...
      fprintf(fp, "    position: %d\n", arg->nodes[i].predecessor.two.position);
      muts = arg->nodes[i].predecessor.one.mutations;
      if (muts != NULL && Length(muts) > 0)
      {
        LListCounter *lcounter = MakeCounter(muts, FIRST);
