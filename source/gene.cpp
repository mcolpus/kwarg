...
/* Allocate and initialise _new set of genes structure with length
...
static char *read_file(FILE *fp)
{
    ...
    while ((data[i] = fgetc(fp)) != EOF)
        if (++i == n)
        {
            ...
            data = static_cast<char *>(xrealloc(data, 2 * n * sizeof(char)));
            n *= 2;
        }

    /* Terminate string */
    data[i] = '\0';
    data = static_cast<char *>(xrealloc(data, (i + 1) * sizeof(char)));
    ...
    while (line != NULL)
    {
        ...
        if ((token->value == '#') && (token->type == CHAR))
        {
            ...
            case 0:
                ...
                {
                    token = static_cast<Gene_Token *>(Pop(line));
                    t[i] = token->value;
                    free(token);
                }
                t[i] = '\0';
...
static AnnotatedGenes *parse_genes_fasta(char *s, Gene_SeqType seqtype)
{
    ...
    while (*s != '\0')
    {
        ...
        /* End of line - skip stretches of _new lines */
        while (*s == '\n')
            s++;
    }
...
void add_ancestral_sites(Sites *s)
{
    ...
    if (index == 0)
        ...
        /* Room for _new sequence in existing bit arrays */
        for (i = 0; i < s->length; i++)
...
{
    Sites *_new = (Sites *)xmalloc(sizeof(Sites));
    int i, blocks = divblocksize(s->n - 1) + 1;

    _new->length = s->length;
    _new->n = s->n;
    _new->data = (Site *)xmalloc(_new->length * sizeof(Site));

    for (i = 0; i < _new->length; i++)
    {
        _new->data[i].type = (unsigned long *)xmalloc(blocks * sizeof(unsigned long));
        memcpy(_new->data[i].type, s->data[i].type, blocks * sizeof(unsigned long));
        _new->data[i].ancestral = (unsigned long *)xmalloc(blocks * sizeof(unsigned long));
        memcpy(_new->data[i].ancestral, s->data[i].ancestral,
               blocks * sizeof(unsigned long));
    }
...
Genes *copy_allbutone(Genes *g, int a)
{
    ...
    {
        _new->data[i].type = (unsigned long *)xmalloc(blocks * sizeof(unsigned long));
        memcpy(_new->data[i].type, g->data[i + 1].type,
               blocks * sizeof(unsigned long));
        _new->data[i].ancestral = (unsigned long *)xmalloc(blocks * sizeof(unsigned long));
        memcpy(_new->data[i].ancestral, g->data[i + 1].ancestral,
               blocks * sizeof(unsigned long));
    }
...
Genes *copy_region(Genes *g, int a, int b)
{
    ...
    for (i = 0; i < _new->n; i++)
    {
        ...
        {
            _new->data[i].type[j] = g->data[i].type[j + start] >> shift;
            _new->data[i].ancestral[j] = g->data[i].ancestral[j + start] >> shift;
            if (shift != 0)
            {
                _new->data[i].type[j] |= g->data[i].type[j + start + 1] << (BLOCKSIZE - shift);
                _new->data[i].ancestral[j] |= g->data[i].ancestral[j + start + 1] << (BLOCKSIZE - shift);
            }
        }
        ...
        if (pattern != 0)
        {
            _new->data[i].type[j] &= pattern;
            _new->data[i].ancestral[j] &= pattern;
        }
    }
 ...
 * index. If _new becomes full, copy back to nblock in g and start a
 * _new block in _new. index and nblock are updated to reflect the changes.
 */
static void transfer_data(Genes *g, int block, int low, int high,
                          unsigned long **_new, int *index, int *nblock)
{
    int i;
    ...
    if (high - low + 1 + *index >= BLOCKSIZE)
    {
        ...
        for (i = 0; i < g->n; i++)
        {
            ...
            if (low == *index)
            {
                /* If low = index, the standard shift to construct _new initial
                 * value of _new would be equal to BLOCKSIZE.
                 */
                g->data[i].type[*nblock] = _new[0][i] | tmp;
                _new[0][i] = 0;
            }
            else
            {
                g->data[i].type[*nblock] = _new[0][i] | tmp << (*index - low);
                _new[0][i] = tmp >> (low + BLOCKSIZE - *index);
            }
            /* Copy ancestral data */
            ...
            if (low == *index)
            {
                 ...
                 * value of _new would be equal to BLOCKSIZE.
                 */
                g->data[i].ancestral[*nblock] = _new[1][i] | tmp;
                _new[1][i] = 0;
            }
            else
            {
                g->data[i].ancestral[*nblock] = _new[1][i] | tmp << (*index - low);
                _new[1][i] = tmp >> (low + BLOCKSIZE - *index);
            }
        }
        /* Update indeces - we have added high - low + 1 _new sites and
    ...
    else
    {
        ...
        if (*index > low)
            ...
            {
                /* Copy type data */
                _new[0][i] |= (g->data[i].type[block] & filter) << (*index - low);
                /* Copy ancestral data */
                _new[1][i] |= (g->data[i].ancestral[block] & filter) << (*index - low);
            }
        else
            ...
            {
                /* Copy type data */
                _new[0][i] |= (g->data[i].type[block] & filter) >> (low - *index);
                /* Copy ancestral data */
                _new[1][i] |= (g->data[i].ancestral[block] & filter) >> (low - *index);
            }
        *index += high - low + 1;
    }
}
...
int remove_siamesetwins(Genes *g, RunData &run_data)
{
    ...
    if (types)
    {
        ...
        if (!run_data.site_labels.empty())
        {
            ...
            for (i = 0; i < n; i++)
            {
                s = -run_data.site_labels[state[i].master];
                if (s <= 0)
                {
                    s = 1;
                }
                for (j = state[i].start; j < state[i].master; j++)
                {
                    ...
                    run_data.site_labels.erase(run_data.site_labels.begin() + j - k);
                    k++;
                }
                kk = k;
                ...
                //                 printf("%d %d\n", state[i].master-kk, -s);
                run_data.site_labels[state[i].master - kk] = -s;
            }
        }
    ...
    for (i = 0; i < blocks; i++)
    {
        ...
        if (((i < blocks - 1) && ~current) || ((i >= blocks - 1) && (current != ((unsigned long)2 << modblocksize(g->length - 1)) - 1)))
        {
            ...
            if (nindex < 0)
            {
                ...
                nblock = i;
                nindex = 0;
                /* Set up structures for building _new gene data */
                ...
                while (~current & index)
                {
                    ...
                    if (!run_data.site_labels.empty())
                    {
                        if (mulblocksize(i) + j < g->length)
                        {
                            ...
                            run_data.site_labels.erase(run_data.site_labels.begin() + mulblocksize(i) + j - n);
                        }
                        else
                        {
                            j = BLOCKSIZE;
...
int coalesce_subsumed(Genes *g, RunData &run_data)
{
                ...
                if (g->data[j].type != NULL)
                {
                    ...
                    if (!(k < blocks))
                    {
                        ...
                        if (!(k < blocks))
                        {
                            ...
                            {
                                run_data.sequence_labels[i] = -1;
                                run_data.sequence_labels[j] = -1;
                            }
    ...
    if (changes > 0)
    {
        ...
        for (j = i + 1; j < g->n; j++)
            if (g->data[j].type != NULL)
            {
                /* This sequence was not assimilated - move it to its _new position */
...
int implode_genes(Genes *g, RunData &run_data)
{
    ...
    while (change)
    {
        ...
        change = remove_uninformative(g, run_data);
        if (g->n == 0)
            break;
        /* Look for removable Siamese twins */
        /* Collapsing Siamese twins should not allow _new mutations, and
         * coalesces are handled presently.
         */
        remove_siamesetwins(g, run_data); // collapses identical neighbouring cols

        /* Look for safe coalesces */
        tmp = coalesce_subsumed(g, run_data);
        change |= tmp;
    }
...
void coalesce(Genes *g, int a, int b, RunData &run_data)
{
    ...
    if (!run_data.sequence_labels.empty())
    {
        vector_swap_elements(run_data.sequence_labels, b, run_data.sequence_labels.size() - 1);
        run_data.sequence_labels.pop_back();
    }
 ...
 */
static void _split(Genes *g, int a, int index, int block, RunData &run_data)
{
    ...
    if (index != 0)
    {
        ...
        /* We need to copy block to the _new sequence */
        j = block;
...
{
    _split(g, a, modblocksize(i), divblocksize(i), run_data);
}
 ...
 */
std::vector<Genes *> _force_split(Genes *g, int a, bool use_events, std::vector<Event> *events, RunData &run_data)
{
...
#endif
            h = copy_genes(g);
            _split(h, a, index, block, run_data);
            forced.push_back(h);
            /* Insert corresponding event in list of events */
            if (use_events)
            {
...
{
    return _force_split(g, a, false, nullptr, run_data);
}

std::vector<Genes *> force_split(Genes *g, int a, std::vector<Event> &events, RunData &run_data)
{
    return _force_split(g, a, true, &events, run_data);
}
...
{
    Index *prefixs = static_cast<Index *>(xmalloc(g->n * sizeof(Index)));
...
{
    Index *postfixs = static_cast<Index *>(xmalloc(g->n * sizeof(Index)));
    int i, j, blocks = divblocksize(g->n - 1) + 1;
 ...
 * graph (so length is g->n), compatibility_matrix is array of edge lists for each node in the compatible
                                     ...
                                     const RunData &main_path_data, STORE_FRAGMENT_FUNCTION_TYPE f)
{
    ...
    else
    {
        ...
        if ((components[current->node] <= 0) && (components[current->node] != -current->_class))
        {
            ...
            int old = components[current->node];
            ...
            if (i == component.size())
            {
                ...
                if (E[current->node].size() > 0)
                {
                    ...
                    for (i = 0; i < E[current->node].size(); i++)
                    {
                        ...
                        {
                            auto node = std::make_unique<NodeClass>();
                            node->node = E[current->node][i];
                            node->_class = current->_class;
                ...
                coalesce_cande_recursion(stack, component, g, E, compatibility_matrix, components, main_path_data, f);
                /* Clean up */
                if (E[current->node].size() > 0)
                {
                    for (i = 0; i < n; i++)
...
void coalesce_compatibleandentangled_map(Genes *g, const RunData &main_run_data, STORE_FRAGMENT_FUNCTION_TYPE f)
{
    ...
    for (i = 0; i < g->n; i++)
        for (j = i + 1; j < g->n; j++)
            if (compatible(g, i, j))
            {
                compatibility_matrix[i][j] = compatibility_matrix[j][i] = 1;
    ...
    for (s = 0; s < g->n; s++)
    {
        if (a[s].index + mulblocksize(a[s].block) > 0)
        {
                ...
                ((a[s].block == b[s].block) && a[s].index < b[s].index))
            {
                ...
                {
                    if ((i == s) ||
                        (((g->data[s].type[a[s].block] ^ g->data[i].type[a[s].block]) & g->data[s].ancestral[a[s].block] & g->data[i].ancestral[a[s].block] & (((unsigned long)2 << a[s].index) - 1)) != 0))
                    {
                ...
                if (sindex < 0)
                {
                     ...
                     */
                    for (i = 0; i < j; i++)
                        ancestral[i] = g->data[out[i]].ancestral[sblock] & (((unsigned long)2 << a[s].index) - 1) & ~(((unsigned long)1 << sindex) - 1);
                }
                else
                {
                    ...
                    for (i = 0; i < j; i++)
                    {
                        if ((g->data[out[i]].ancestral[a[s].block] & (((unsigned long)2 << a[s].index) - 1)) != 0)
                        {
...
{
    HashGenesParameters *p = static_cast<HashGenesParameters *>(xmalloc(sizeof(HashGenesParameters)));
    p->m = m;
...
Genes *unpack_genes(PackedGenes *p)
{
    ...
    /* Unpack p into the _newly allocated structure */
    pblock = -1;
...
static HashPackedGenesParameters *initialise_hashpackedparameters(unsigned long m)
{
    ...
    HashPackedGenesParameters *p = static_cast<HashPackedGenesParameters *>(xmalloc(sizeof(HashPackedGenesParameters)));
    p->m = m;
